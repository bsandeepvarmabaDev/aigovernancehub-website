#!/usr/bin/env node
/** Package AI Governance Hub website v25.8 */
import fs from "fs";
...
const outZip = path.join(root, "aigovernancehub-website-v25.8.zip");
const exclude = new Set([
  "node_modules", ".env", ".env.local", ".vercel",
  "aigovernancehub-website-v25.7.zip", "aigovernancehub-website-v25.8.zip",
  "aigovernancehub-website-v26.0.zip",
  ".v25.7-staging", ".v25.8-staging", ".v26-staging",
]);

function shouldInclude(rel) {
  return !rel.split(/[/\\]/).some((p) => exclude.has(p) || p.endsWith(".zip"));
}
...
const staging = path.join(root, ".v25.8-staging");
